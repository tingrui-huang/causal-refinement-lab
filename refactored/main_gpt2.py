"""
Main Program: GPT-2 (Small Model Baseline)

This is for running causal discovery using GPT-2.
GPT-2 is a smaller, older model for comparison with larger LLMs.

Good for evaluating: Does model size matter for causal reasoning?
"""
...
from modules.api_clients import GPT2Client
from modules.scanners import MutualInformationScanner
from modules.prompt_generators import DirectPromptGenerator
...
class CausalDiscoveryPipeline:
    def __init__(self, data_path, model_id="gpt2-large", output_dir="outputs"):
        print("=" * 60)
        print("Initializing Causal Discovery Pipeline (GPT-2)")
        print("=" * 60)
        ...
        print("\n[2/8] Loading GPT-2 model locally (CPU)...")
        self.api_client = GPT2Client(model_id)
        ...
        self.prompt_generator = DirectPromptGenerator(self.data_loader)
        print("[PROMPT] Using Direct prompting (GPT-2 has limited context)")
        ...
        self.model_name = f"gpt2_{model_id.replace('-', '_')}"
    ...
    def run(self, max_steps=50, mi_threshold=0.05, significance_level=0.05):
        ...
        while step < max_steps:
            ...
            response = self.api_client.call(prompt, max_tokens=100)
...
def main():
    print("\n" + "=" * 60)
    print("Causal Discovery with GPT-2 (Local CPU)")
    print("=" * 60)
    
    print("\n" + "!" * 60)
    print("ABOUT GPT-2:")
    print("- Smaller, older model (up to 1.5B parameters)")
    print("- Runs locally on CPU (no GPU needed)")
    print("- No API key required!")
    print("- Good baseline for comparing with larger models")
    print("- Question: Does model size matter for causal reasoning?")
    print("!" * 60)
    
    print("\n" + "!" * 60)
    print("MODEL VARIANTS:")
    print("  1. gpt2        - 124M parameters (smallest)")
    print("  2. gpt2-medium - 355M parameters")
    print("  3. gpt2-large  - 774M parameters [DEFAULT]")
    print("  4. gpt2-xl     - 1.5B parameters (largest)")
    print("!" * 60)
    
    # Get model variant
    model_input = input("\nModel variant (default: gpt2-large): ").strip()
    model_id = model_input if model_input in ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'] else 'gpt2-large'
    ...
    pipeline = CausalDiscoveryPipeline(data_path, model_id, output_dir="../outputs")
    ...
    print("Compare with GPT-3.5/Gemini to see model size impact!")
    print("=" * 60 + "\n")
