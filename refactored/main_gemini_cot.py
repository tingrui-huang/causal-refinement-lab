"""
Main Program: Gemini with Chain-of-Thought
...
from modules.api_clients import GeminiClient
...
class CausalDiscoveryPipeline:
    def __init__(self, data_path, api_key, model_name="models/gemini-2.0-flash-exp", output_dir="outputs"):
        print("=" * 60)
        print("Initializing Causal Discovery Pipeline (Gemini)")
        print("=" * 60)
        ...
        print("\n[2/8] Connecting to Gemini API...")
        self.api_client = GeminiClient(api_key, model_name)
...
def main():
    api_key = input("\nPlease enter your Google AI API key: ").strip()
    ...
    # Get model name
    model_input = input("\nModel name (default: models/gemini-2.0-flash-exp): ").strip()
    model_name = model_input if model_input else "models/gemini-2.0-flash-exp"
    ...
    pipeline = CausalDiscoveryPipeline(data_path, api_key, model_name, output_dir="../outputs")
