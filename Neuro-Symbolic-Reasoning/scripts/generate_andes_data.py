"""
Generate Andes dataset for causal discovery

The Andes dataset is a Bayesian network for intelligent tutoring systems
in physics problem solving.

Data source: bnlearn library (preferred) or BIF file
Ground truth: 338 edges for physics tutoring

Reference: ANDES intelligent tutoring system (University of Pittsburgh)
"""
...
def load_andes_from_bnlearn():
    """
    Try to load Andes dataset from bnlearn library
    ...
    print("=" * 80)
    print("LOADING ANDES DATASET FROM BNLEARN")
    print("=" * 80)
    
    try:
        # Try to load data
        print("\n[1/3] Loading Andes dataset...")
        df = bn.import_example('andes')
...
def load_andes_from_bif(bif_path, size=10000):
    """
    Load Andes dataset from BIF file and generate samples
    
    Args:
        bif_path: Path to andes.bif file
        size: Number of samples to generate (default: 10000 for sufficient FCI performance)
    ...
    print("=" * 80)
    print("LOADING ANDES DATASET FROM BIF")
    print("=" * 80)
    ...
    # Generate samples
    print(f"[3/3] Generating {size} samples...")
    print(f"  (This may take a while for large networks...)")
    sampler = BayesianModelSampling(model)
    df = sampler.forward_sample(size=size, show_progress=False)
...
def create_ground_truth_file(edges, output_path):
    ...
    with open(output_path, 'w') as f:
        f.write("# Andes intelligent tutoring system ground truth\n")
        f.write("# Source: ANDES physics tutoring system (University of Pittsburgh)\n")
        f.write(f"# {len(edges)} edges for physics problem solving\n\n")
...
def create_metadata(variables, state_mappings, n_samples, output_path):
    ...
    metadata = {
        ...
        "n_samples": n_samples,
        "domain": "intelligent_tutoring",
        "source": "ANDES intelligent tutoring system (University of Pittsburgh)",
        "description": "Andes network for physics problem solving tutoring",
        "reference": "ANDES ITS - Physics tutoring expert system"
    }
    ...
    print(f"  Domain: Intelligent tutoring (physics problem solving)")


def main():
    """Main function to generate all Andes dataset files"""
    
    print("\n" + "=" * 80)
    print("GENERATING ANDES DATASET")
    print("=" * 80)
    print("Purpose: Intelligent tutoring system benchmark for causal discovery")
    print("Strategy: 223 variables, discrete states, expert-validated DAG")
    print("Sample size: 10000 (required for sufficient FCI performance)")
    print("=" * 80)
    
    # Setup paths
    base_dir = Path(__file__).parent.parent
    data_dir = base_dir / 'data' / 'andes'
    data_dir.mkdir(parents=True, exist_ok=True)
    
    # Sample size (10000 for sufficient FCI performance)
    SAMPLE_SIZE = 10000
    
    # Try to load from bnlearn first (preferred)
    df, edges, variables = load_andes_from_bnlearn()
    
    # If bnlearn fails, try from BIF file
    if df is None:
        print("\n[INFO] Trying to load from BIF file...")
        bif_path = data_dir / 'andes.bif'
        
        if not bif_path.exists():
            print("\n" + "=" * 80)
            print("ERROR: Andes dataset not available")
            print("=" * 80)
            print("\nPlease download andes.bif from:")
        ...
        df, edges, variables = load_andes_from_bif(bif_path, size=SAMPLE_SIZE)
    
    if df is None:
        print("\n[ERROR] Failed to load Andes dataset")
        return
    ...
    print("=" * 80)
    onehot_path = data_dir / 'andes_data.csv'
    ...
    print("=" * 80)
    variable_path = base_dir.parent / 'andes_data_variable.csv'
    ...
    print("=" * 80)
    gt_path = data_dir / 'andes_ground_truth.txt'
    ...
    print("\n" + "=" * 80)
    print("[SUCCESS] ANDES DATASET GENERATION COMPLETE!")
    print("=" * 80)
    print("\nGenerated files:")
