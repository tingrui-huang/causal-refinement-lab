"""
Generate Hailfinder dataset for causal discovery

The Hailfinder dataset is a meteorological Bayesian network for forecasting
severe summer hail in northeastern Colorado.

Data source: BIF file from bnlearn repository
Ground truth: 66 edges validated by meteorological experts

Reference: Abramson et al. (1996)
"""
...
def load_hailfinder_from_bif(bif_path):
    """
    Load Hailfinder dataset from BIF file and generate samples
    
    Args:
        bif_path: Path to hailfinder.bif file
    ...
    print("=" * 80)
    print("LOADING HAILFINDER DATASET FROM BIF")
    print("=" * 80)
...
def convert_to_onehot(df, output_path):
    ...
    for col in df.columns:
        ...
        if len(df.columns) <= 30:  # Only print details for smaller datasets
            print(f"  {col:25s}: {n_states} states -> {len(onehot.columns)} columns")
...
def convert_to_variable_level(df, output_path):
    ...
    for col in df.columns:
        ...
        if len(df.columns) <= 30:  # Only print details for smaller datasets
            print(f"  {col:25s}: {df[col].nunique()} unique values -> codes 0-{df_numeric[col].max()}")
...
def create_ground_truth_file(edges, output_path):
    ...
    with open(output_path, 'w') as f:
        f.write("# Hailfinder meteorological network ground truth\n")
        f.write("# Source: Meteorological expert knowledge\n")
        f.write(f"# {len(edges)} edges for severe hail forecasting\n")
        f.write("# Reference: Abramson et al. (1996)\n\n")
...
def create_metadata(variables, state_mappings, n_samples, output_path):
    ...
    metadata = {
        ...
        "n_samples": n_samples,
        "domain": "meteorology",
        "source": "Bayesian network from meteorological expert knowledge",
        "description": "Hailfinder network for forecasting severe summer hail in northeastern Colorado",
        "reference": "Abramson, B., Brown, J., Edwards, W., Murphy, A., & Winkler, R.L. (1996). Hailfinder: A Bayesian system for forecasting severe weather. International Journal of Forecasting, 12(1), 57-71."
    }
    ...
    print(f"  Domain: Meteorology (severe hail forecasting)")


def main():
    """Main function to generate all Hailfinder dataset files"""
    
    print("\n" + "=" * 80)
    print("GENERATING HAILFINDER DATASET")
    print("=" * 80)
    print("Purpose: Meteorological benchmark for causal discovery")
    print("Strategy: 56 variables, discrete states, expert-validated DAG")
    print("=" * 80)
    
    # Setup paths
    base_dir = Path(__file__).parent.parent
    data_dir = base_dir / 'data' / 'hailfinder'
    data_dir.mkdir(parents=True, exist_ok=True)
    
    # Load from BIF file
    bif_path = data_dir / 'hailfinder.bif'
    
    if not bif_path.exists():
        print("\n" + "=" * 80)
        print("ERROR: Hailfinder dataset not available")
        print("=" * 80)
        print("\nPlease download hailfinder.bif from:")
    ...
    df, edges, variables = load_hailfinder_from_bif(bif_path)
    
    if df is None:
        print("\n[ERROR] Failed to load Hailfinder dataset")
        return
    ...
    print("=" * 80)
    onehot_path = data_dir / 'hailfinder_data.csv'
    ...
    print("=" * 80)
    variable_path = base_dir.parent / 'hailfinder_data_variable.csv'
    ...
    print("=" * 80)
    gt_path = data_dir / 'hailfinder_ground_truth.txt'
    ...
    print("\n" + "=" * 80)
    print("[SUCCESS] HAILFINDER DATASET GENERATION COMPLETE!")
    print("=" * 80)
    print("\nGenerated files:")
