...
def main():
    ...
    if len(history['unresolved_ratio']) > 0:
        ...
        print(
            f"  Active Blocks: {history['active_blocks'][0]}/{history['active_blocks'][0]} -> {history['active_blocks'][-1]}/{len(priors['blocks'])}")
        print("=" * 80)
    print()
    ...
    print(
        f"  Prior building: {timing['prior_building']:.2f}s ({timing['prior_building'] / timing['total'] * 100:.1f}%)")
